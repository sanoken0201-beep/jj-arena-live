...
def main() -> None:
    ...
    # Hotkeys are opt-in, modifier-only, and never confirm a chip-committing action.
    assert "localStorage.getItem(jjV3UserKey('hotkeys'))==='1'" in js5
    assert "!e.ctrlKey||e.altKey||e.metaKey||e.shiftKey||e.repeat" in js5
    ...
    assert "e.code==='KeyF'" in hot_code and 'data-action=\"fold\"' in hot_code
    assert "e.code==='KeyK'" in hot_code and 'data-action=\"check\"' in hot_code
    assert "e.code==='KeyR'" in hot_code and "target.focus" in hot_code
    assert "Digit[1-4]" in hot_code
    assert 'data-action=\"call\"' not in hot_code
    assert 'data-action=\"raise\"' not in hot_code
    assert "doAction(" not in hot_code
    assert "Ctrlホットキーを有効にする" in js5
    assert "コール・ベット・レイズの確定は行いません" in js5
