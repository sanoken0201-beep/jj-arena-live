...
def _load_engine(root: Path):
    ...
    spec = importlib.util.spec_from_file_location("jj_v123_engine_integration", path)
...
def test_real_no_flop_no_drop(engine) -> None:
    state = _new_table(engine, "v123-nfnd", [1000, 1000])
...
def test_real_three_way_sidepot_runout(engine) -> None:
    state = _new_table(engine, "v123-sidepot", [500, 1000, 1500])
    # With seats 0/1/2 and the initial button advancing to seat 0, the action
    # order is seat 0 -> seat 1 -> seat 2. This creates 500/1000/1000 committed
    # chips and leaves seat 2 with an unmatched 500 behind.
...
def main() -> None:
    ...
    print("v1.23 real engine integration: ok")
