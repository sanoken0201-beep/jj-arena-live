...
def _server(path: Path) -> None:
    ...
    text = text.replace('"version":"1.20.2"', '"version":"1.20.3"')
    text = text.replace('request.url.query == "v=47"', 'request.url.query == "v=48"')
...
def _app(path: Path) -> None:
  ...
  renderQuiz=function(){
    ...
    $('#quizChoices').innerHTML=(q.choices||[]).map(c=>`<button data-quiz="${safe(String(c.value))}">${safe(c.label)}</button>`).join('');
    $('#quizScore').textContent=`今日 ${p.correct}正解 / ${p.answered}回答 · +${p.earned}pt`;
  };

  answerQuiz=async function(v){
    if(jjV186QuizBusy||!quiz.q?.id||quiz.q?.done)return;
    jjV186QuizBusy=true;
...
def _index(path: Path) -> None:
    text = path.read_text(encoding="utf-8").replace("?v=47", "?v=48")
    path.write_text(text, encoding="utf-8")


def _sw(path: Path) -> None:
    text = path.read_text(encoding="utf-8")
    text = re.sub(r"jj-arena-live-v\d+", "jj-arena-live-v48", text)
    path.write_text(text, encoding="utf-8")
