...
def main() -> None:
    ...
    # Actual legal preset totals must be visible despite the older v1.24 hide rule.
    ...
    assert old_hide in css4, "the regression fixture must still contain the old hide rule"
    assert phase4_show in css4
    assert css4.rfind(phase4_show) > css4.rfind(old_hide), "phase 4 visibility override must win the cascade"

    # Settings are not a live betting-size choice anymore.
    ...
    # Tablet/mobile users retain access to both auxiliary table surfaces.
    ...
    # Mobile numeric information is explicitly raised above the former tiny sizes.
    ...
    assert "transform_phase4_app_js(transform_phase3_app_js" in app_source
    assert "transform_phase4_styles(transform_phase3_styles" in app_source
    assert "/static/styles.css?v=61" in app_source
    assert "/static/app.js?v=61" in app_source
    assert "jj-arena-live-v61" in app_source
