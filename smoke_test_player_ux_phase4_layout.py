...
<style>.fixture-seat{position:absolute;left:50%;top:79%;transform:translate(-50%,-50%);z-index:6}.fixture-seat .jj-seat-box{width:108px}.fixture-seat .jj-hole{display:flex;justify-content:center}.fixture-seat .card-face{width:38px;height:53px}#tableSidePanel,#jjSideDrawerBackdrop{transition:none!important}</style></head>
<body>
...
def _run(chrome: str, fixture: Path, width: int, height: int, *, mobile: bool, keyboard: bool, drawer: bool) -> str:
    ...
    proc = subprocess.run(
        [chrome, "--headless", "--no-sandbox", "--disable-gpu", "--allow-file-access-from-files", "--run-all-compositor-stages-before-draw", "--virtual-time-budget=3000", f"--window-size={width},{height}", "--dump-dom", fixture.resolve().as_uri() + q],
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        for width, height, mobile, keyboard, drawer in cases:
            ...
            assert 'data-ok="1"' in dom, f"phase4 layout failed {width}x{height} mobile={mobile} keyboard={keyboard} drawer={drawer}: {dom[-2600:]}"
