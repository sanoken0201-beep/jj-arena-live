(()=>{
  ...
  function ensureEditor(){
    ...
    note.insertAdjacentHTML('beforebegin',`<section class="sng-config-editor"><div class="sng-config-head"><div><b>トーナメント設定</b><small>大会開始後は変更できません</small></div></div><label>参加ポイント<input id="sngEntryFee" type="number" min="0" max="1000000" step="1" inputmode="numeric" value="0" required><small>0なら無料。参加登録時に徴収し、開始前キャンセル・大会中止は自動返金します。</small></label><label>初期スタック<input id="sngStartingStack" type="number" min="1000" max="10000000" step="100" inputmode="numeric" value="30000" required></label><div class="sng-structure-head"><div><b>ブラインドストラクチャー</b><small>SB / BB / BBA / 各レベル時間を大会ごとに設定</small></div><button id="sngAddLevel" type="button" class="soft">＋ レベル追加</button></div><div class="sng-level-editor-head"><span>Lv</span><span>SB</span><span>BB</span><span>BBA</span><span>分</span><span></span></div><div id="sngStructureEditor" class="sng-level-editor"></div><p class="field-note compact">最終レベル到達後は、そのレベルを大会終了まで継続します。チップ値は100点単位です。</p></section>`);
  ...
  function collectConfig(){
    const entry_fee_points=Number($('#sngEntryFee')?.value);if(!Number.isInteger(entry_fee_points)||entry_fee_points<0||entry_fee_points>1000000)throw new Error('参加ポイントは0〜1,000,000ptの整数で設定してください');
    ...
    return {entry_fee_points,starting_stack,structure};
  }
  ...
  function render(){
    ...
    const d=data.defaults;if(d){
      ...
      if(!editorInitialized&&!editingId){$('#sngEntryFee').value=d.entry_fee_points||0;$('#sngStartingStack').value=d.starting_stack;renderEditor(d.structure);editorInitialized=true}
    }
  }
  async function load(){data=await api('/admin/sitngo');render()}
  function beginEdit(id){
    const e=(data.events||[]).find(x=>x.id===id);if(!e)return;
    editingId=id;ensureEditor();$('#sngName').value=e.name;$('#sngStartsAt').value=localInput(e.starts_at);$('#sngEntryFee').value=e.entry_fee_points||0;$('#sngStartingStack').value=e.starting_stack;renderEditor(e.structure);$('#sngSubmit').textContent='変更を保存';$('#sngCancelEdit').hidden=false;render();$('#sngCreateForm').scrollIntoView({behavior:'smooth',block:'start'});
  }
  function endEdit(reset=true){
    editingId=null;const form=$('#sngCreateForm');if(reset&&form){$('#sngName').value='JJ Sit&Go';$('#sngStartsAt').value=defaultStart();$('#sngEntryFee').value=data.defaults?.entry_fee_points||0;$('#sngStartingStack').value=data.defaults?.starting_stack||30000;renderEditor(data.defaults?.structure||fallbackStructure)}$('#sngSubmit').textContent='開催を設定';$('#sngCancelEdit').hidden=true;render();
  }
  function activate(){
